    ...
    <form class="layui-form " action="" lay-filter="dataFrm" id="dataFrm">
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="ubirthday" id="ubirthday" lay-verify="required" autocomplete="off"
                           class="layui-input">
                </div>
            </div>
        </div>
...
<script >
    layui.use([ 'table','form'], function(){
        ...
        table.render({
            ...
            ,toolbar:"#userToolBar"
            ,id: 'docReload'
            ,url: '<%=path%>admin/vipUserSelect.action' //数据接口
            ,page: true //开启分页
        ...
        table.on("toolbar(test)",function(obj){
            switch(obj.event){
                ...
                case 'getCheckData':
                    ...
                    alert(params);
                    layer.confirm('确定更改选中的这些vip用户为普通用户', function(index){
                        sendDelectData("<%=path%>admin/deleteVipUsers.action",params);
                        layer.close(index);
                    });
                    break;
            };
        ...
        function openUpdateUser(data){
            mainIndex=layer.open({
                ...
                success:function(index){
                    ...
                    url="<%=path%>admin/updateVipUser.action";
                }
            });
        }
        //保存
        ...
        table.on('tool(test)', function(obj) {
            ...
            alert(data.userid);
            if (obj.event === 'delete') {
                layer.confirm('确定从vip用户中移除，变为普通用户？', function (index) {
                    fal("<%=path%>admin/deleteVipUser.action",data.userid);
                    layer.close(index);
                });
            }else if (obj.event === 'resetpwd') {
                layer.confirm('确定重置密码为123456？', function (index) {
                    fal("<%=path%>admin/vipResetPwd.action",data.userid);
                    layer.close(index);
                });
