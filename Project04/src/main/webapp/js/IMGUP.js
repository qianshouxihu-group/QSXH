...
var IMG_LENGTH = 5;//图片最大1MB
var IMG_MAXCOUNT =1;//最多选中图片张数
var IMG_AJAXPATH = "uploadManager/pictureManager.action";//异步传输服务端位置
...
$("#div_imgfile").click(function () {
    ...
    if ($(".imgfile").length <= $(".lookimg").length) {//个数不足则新创建对象
        ...
        _CRE_FILE.setAttribute("accept", ".png,.jpg,.jpeg,.gif");
            ...
            success: function (data) {
                if (data.isok == "1") {
                    ...
                    $(".lookimg").eq([data.ind]).children(".lookimg_progress").eq(0).children("div").eq(0).animate({ width: "100%" }, UPTIME, function () {
                        ...
                        window.location.href ='personalManager/myPhotograph.action';
                    });
